...
if(isset($_POST['submit'])) {
  ...
  if($email != "" && $password != "") {
    try {
      $query = "select id, fname, lname, email, mobile, password, address, create_date, gender, age, height, weight from tbluser where email=:email and password=:password";
      ...
      if($count == 1 && !empty($row)) {
		...
		if (isset($row['gender']) && isset($row['age']) && isset($row['height']) && isset($row['weight'])) {
			$query = "select count(*) from tblbooking where userid=:userId";
			$stmt = $dbh->prepare($query);
			$stmt->bindParam('userId', $row['id'], PDO::PARAM_STR);
			$stmt->execute();
			$countTemp = $stmt->rowCount();
			$row   = $stmt->fetch(PDO::FETCH_ASSOC);
			// if($countTemp > 0){
			// 	print_r('workout plans here sunday monday wala'); die;
			// }else{
		...
		<div class="container">
			<div class="row">
				<div class="col-lg-7 m-auto text-white">
					<div>
						Your Personalized Path to Wellness. Tailored workouts and diet plans,
						 personalized for your goals and preferences. 
						 Track your progress, access expert knowledge, 
						 and connect with a supportive community. 
						 Make your healthy living journey enjoyable and achievable. 
